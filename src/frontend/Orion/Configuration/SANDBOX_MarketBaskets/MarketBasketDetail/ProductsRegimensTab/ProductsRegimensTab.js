...
const ProductsRegimensTab = () => {
  ...
  return (
    <div>
      ...
      />
      {products_regimens.length && <UpdateForm
        product={products_regimens[0].product}
      />}
    </div>
  )
}
...
const getTableData = (products_regimens) => {
  const groupedProdRegCombos = _.groupBy(products_regimens, 'product.generic_name')
  const tableData = Object.entries(groupedProdRegCombos).reduce((acc, [genericName, regimenCombos]) => {
    const regimens = regimenCombos.map(({ regimen: { name } }) => name).join(', ')
