...
import { GET_MARKET_BASKETS, GET_VEGA_PRODUCTS_REGIMENS } from 'frontend/api/queries'
...
const UpdateForm = ({ product }) => {
  ...
  const [marketBasketProdRegComboIds, setMarketBasketProdRegComboIds] = useState([])
  const [selectedRegimenComboOptions, setSelectedRegimenComboOptions] = useState([])
  ...
  useEffect(() => {
    if (!loading) {
      ...
      const selectedProductRegimenComboOptions = products_regimens
        .filter(({ product: { id: localProdId } }) => localProdId === product.id)
        .map(({ id, regimen: { name } }) => ({
          label: name,
          value: Number(id),
        }))
  ...
  const [updateMarketBasket, { loading: isUpdating }] = useMutation(UPDATE_MARKET_BASKET,
    {
      variables: {
        input: {
          id: marketBasketId,
  ...
  const handleRegimensOnChange = (options) => {
    ...
    const toAdd = _.difference(selectedRegimenCombos, selectedRegimenComboOptions)
      .map(({ value }) => value)
    const toRemove = _.difference(selectedRegimenComboOptions, options)
      .map(({ value }) => value)
    const filteredMarketBasketProdRegComboIds = marketBasketProdRegComboIds
