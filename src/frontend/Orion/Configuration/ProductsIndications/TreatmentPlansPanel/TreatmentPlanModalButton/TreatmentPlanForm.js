...
const getSelectDropdowns = (
  [state, setState],
  treatmentPlanParts,
) => {
  const sortedEntries = _.sortBy(Object.entries(treatmentPlanParts), ([part]) => PART_ORDER[part])

  return sortedEntries.map(([part, data]) => {
    const partSingular = part.slice(0, part.length - 1)

    const dropdownOptions = data.map(datum => ({ label: datum.name, value: datum }))

    let selectedValue = null
    if (state[partSingular]) {
      selectedValue = { value: state[partSingular], label: state[partSingular].name }
    }
