...
const PHOENIX_NOTE =
  'If you add a new indication and / or regimen to the Treatment Plan list, make sure to add them to the Phoenix Indication + Regimen list so that they are able to be activated on Phoenix.'
        ...
        treatmentPlans.map(({ _id, ...tpParts }) => {
          ...
          return (
            <div key={_id} style={{ display: 'flex' }}>
              ...
              <span style={rowStyle}>{tpParts.regimen}</span>
              <span style={rowStyle}>{tpParts.population}</span>
              <span style={rowStyle}>{tpParts.line}</span>
              <span style={rowStyle}>{tpParts.book}</span>
              <span style={rowStyle}>{tpParts.coverage}</span>
