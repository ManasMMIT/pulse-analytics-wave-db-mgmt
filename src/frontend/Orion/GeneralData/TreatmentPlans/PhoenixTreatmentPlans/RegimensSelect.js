...
import { GET_SOURCE_REGIMENS } from '../../../../api/queries'
...
const RegimensSelect = ({ state, handleChange }) => {
  ...
  // TODO: also make searchable by products and their tags, not just regimen name
  const options = regimens.map(({ _id, name }) => ({ value: _id, label: name }))
  ...
  return (
    <Select
      ...
      onChange={(arrOfVals) => {
        ...
        newRegimens = newRegimens.map(({ value }) =>
          stripTypename(regimensByKey[value])
        )
