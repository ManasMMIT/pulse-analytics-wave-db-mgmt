...
const PlacardView = () => {
  ...
  const { data: pqlObjectData, loading: pqlObjectLoading } = usePqlObject(pql)

  const businessObjectName =
    pql.match(/[\w\s]+={.*}/) && pql.match(/[\w\s]+=/)[0].replace('=', '')

  const options = (aquilaBusinessObjects || []).map(({ boName, boId }) => ({
    label: boName,
    value: boId,
  }))
  ...
  if (businessObjectName) {
    selectedOption = options.find(
      ({ label: boName }) => boName === businessObjectName
    )
  }

  useEffect(() => {
    const shouldClean =
      !filtersState.length && !_.isEmpty(boFilterSettings) && !pqlObjectLoading
  ...
  useEffect(() => {
    const cleanFiltersState = getCleanFiltersState(
      boFilterSettings,
      filtersState
    )
        ...
        refetchQueries={[
          {
            ...
            variables: { boId: (selectedOption || {}).value },
          },
...
const getCleanFiltersState = (boFilterSettings, filtersState) => {
  const boFilterFieldsByKey = _.keyBy(boFilterSettings.fields, 'boFieldKey')

  const cleanFiltersState = filtersState.map(({ key, options }) => {
    if (!boFilterFieldsByKey[key]) return { key, options }

    const {
      inputProps: { options: filterOptions },
    } = boFilterFieldsByKey[key]
...
const addNewOptions = ({ key, options }, datum) => {
  ...
  const isInOptions = options.find(
    ({ value: filterOptionValue }) => filterOptionValue === datumValue
  )
