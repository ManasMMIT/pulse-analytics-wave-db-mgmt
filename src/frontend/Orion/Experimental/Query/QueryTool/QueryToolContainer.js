...
import { FILTER_QUERY } from '../../../../api/mutations'
...
const getAccountOnChangeHandler = (orgTypes, history, setSelectedAccount) => (
  account
) => {
...
const getOrgTypesOnChangeHandler = (selectedAccount, history, setOrgTypes) => (
  orgTypes
) => {
  const orgTypesObj = orgTypes
    ? { orgTypes: orgTypes.map(({ value }) => value) }
...
const QueryToolContainer = ({ location: { search }, history }) => {
  // TODO: replace with useEffect to only load URL filters on component mount
  const [hasLoadedInitialFilter, setHasLoadedInitialFilter] = useState(false)
  ...
  const [filterQuery] = useMutation(FILTER_QUERY, {
    onCompleted: ({ filterQuery: result }) => {
      setDataToDisplay(result)
      const shouldNotDisplayData =
        _.isEmpty(result) || _.isEmpty(selectedAccount) || _.isEmpty(orgTypes)
    },
  })
  ...
  const accountFilterOptions = queryToolAccounts.map(
  ...
  const orgTypeFilterOptions = ACCOUNT_TYPE_OPTIONS.map((accountType) => ({
    value: accountType,
    label: accountType,
  }))
  ...
  if (UrlInput.orgTypes) {
    ...
    UrlOrgTypes =
      typeof UrlInput.orgTypes === 'string'
        ? [UrlInput.orgTypes]
        : UrlInput.orgTypes
  ...
  if (UrlInput.selectedAccountId) {
    defaultSelectedAccount = accountFilterOptions.find(
      ({ value }) => value === UrlInput.selectedAccountId
    )
  }
  ...
  if (!hasLoadedInitialFilter) {
    ...
    const formattedUrlOrgTypes = UrlOrgTypes.map((orgType) => ({
      label: orgType,
      value: orgType,
    }))
  ...
  let formattedOrgTypes = orgTypes ? orgTypes.map(({ value }) => value) : []

  const accountConfig = {
    defaultValue: defaultSelectedAccount,
    selected: selectedAccount,
    filterOptions: accountFilterOptions,
    onChangeHandler: getAccountOnChangeHandler(
      orgTypes,
      history,
      setSelectedAccount
    ),
  ...
  const orgTypesConfig = {
    ...
    onChangeHandler: getOrgTypesOnChangeHandler(
      selectedAccount,
      history,
      setOrgTypes
    ),
