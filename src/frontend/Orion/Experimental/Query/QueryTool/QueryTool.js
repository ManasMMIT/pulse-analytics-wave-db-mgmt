    ...
    <PageWrapper>
      <QueryControlsContainer>
        ...
        <QueryControls>
          ...
          <Question>
            {`${
              orgTypesConfig.selected && orgTypesConfig.selected.length > 1
                ? 'are'
                : 'is'
            } affiliated with`}
          </Question>
            ...
            <TableWrapper>
              <TableBody>
                {dataToDisplay.map((result, idx) => {
                  ...
                  const formattedStates = result.connections
                    ? result.connections
                        .reduce((acc, { state }) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableColumn>{result.type}</TableColumn>
                      <TableColumn>{result.oncologistsCount}</TableColumn>
                      <TableColumn>{formattedStates}</TableColumn>
                    </TableRow>
                  )
                })}
              </TableBody>
