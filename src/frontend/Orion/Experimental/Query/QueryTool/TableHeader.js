...
const TableHeader = ({ label, sortConfig }) => {
  const hasSortConfig = !_.isEmpty(sortConfig)
  const [sortOrder, handleSortOrder] = useState('asc')

  const arrowIcon = sortOrder === 'asc' ? faCaretUp : faCaretDown
  ...
  if (hasSortConfig) {
    const { tableData, setDataToDisplay, key } = sortConfig
  ...
  return (
    <TableHeaderItem onClick={handleClick}>
      ...
      {hasSortConfig && (
        <FontAwesomeIcon icon={arrowIcon} color={Colors.PRIMARY} />
      )}
    </TableHeaderItem>
  )
}
