...
import { FormLabel, FieldContainer } from '../../shared/styledComponents'
...
const BusinessObjectValidationForm = ({
  ...
  useEffect(() => {
    ...
    if (!loading && stagedBusinessObjRef) {
      ...
      const selectedBo = businessObjects.find(
        ({ _id }) => _id === stagedBusinessObjRef._id
      )
  ...
  const handleBoChange = (selectedBo) => {
    if (selectedBo) {
      ...
      const newSelectedBo = data.businessObjects.find(
        ({ _id }) => _id === value
      )
      ...
      // user allowed to clear the bo selection; clearing it clears both dropdowns and checkbox
  ...
  const handleBoFieldChange = ({ value }) => {
    const updatedBoRef = Object.assign({}, stagedBusinessObjRef, {
      fieldId: value,
    })
  ...
  const formattedBoFields = businessObjFields.map(({ _id, key }) => ({
    value: _id,
    label: key,
  }))
  ...
  if (!loading) {
    const { businessObjects } = data
    formattedBoObjs = businessObjects.map(({ _id, name }) => ({
      value: _id,
      label: name,
    }))
    ...
    if (stagedBusinessObjRef) {
      ...
      const selectedBoField = selectedBo.fields.find(
        ({ _id }) => _id === stagedBusinessObjRef.fieldId
      )
      formattedSelectedBoField = {
        value: selectedBoField._id,
        label: selectedBoField.key,
      }
    }
  }
  ...
  return (
    <div
      style={{
        borderRadius: 4,
        background: transparentize(0.4, Color.WHITE),
        marginBottom: 24,
        padding: 16,
      }}
    >
      <h4
        style={{
          ...
          fontWeight: 700,
          color: transparentize(0, Color.BLACK),
          margin: 0,
        }}
      >
      ...
      <div
        style={{ color: transparentize(0.5, Color.BLACK), ...FontSpace.FS2 }}
      >
        ...
        <div style={{ marginTop: 24 }}>
          ...
          <div style={{ display: 'flex' }}>
            <input
              ...
              checked={Boolean(
                stagedBusinessObjRef && stagedBusinessObjRef.allowBlankValues
              )}
              type="checkbox"
              onChange={handleCheckboxChange}
              style={{
                marginRight: 12,
                cursor: stagedBusinessObjRef ? 'pointer' : 'not-allowed',
              }}
            />
