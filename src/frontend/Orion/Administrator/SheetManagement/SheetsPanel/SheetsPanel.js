...
const SheetsPanel = () => {
  ...
  const { sheetId: selectedSheetId, workbookId: selectedWorkbookId } =
  ...
  const selectedWorkbook = data.workbooks.find(
    ({ _id }) => _id === selectedWorkbookId
  )
          ...
          >
            ...
            <DeleteButton
              mutationVars={{
                ...
                sheetId: sheetObj._id,
              }}
              mutationDoc={DELETE_SHEET}
              afterMutationHook={() => {
                const targetWorkbook = data.workbooks.find(
                  ({ _id }) => _id === selectedWorkbookId
                )
                const nextSheetSelection = targetWorkbook.sheets.find(
                  ({ _id }) => _id !== sheetObj._id
                )
