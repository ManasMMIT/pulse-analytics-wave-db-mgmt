...
const getSearchParams = (aquilaConfigObj) => {
  const aquilaConfigIdObj = aquilaConfigObj
    ? { aquilaConfigId: aquilaConfigObj._id }
    : {}

  const fieldIdObj =
    aquilaConfigObj && aquilaConfigObj.fields[0]
...
const AquilaConfigsPanel = () => {
  ...
  const handleClick = (aquilaConfigObj) => {
    history.push({
      search: queryString.stringify(getSearchParams(aquilaConfigObj)),
    })
  }
        ...
        {data.aquilaConfigs.map((aquilaConfigObj) => (
          ...
          >
            ...
            <DeleteButton
              ...
              afterMutationHook={() => {
                const nextSelection = data.aquilaConfigs.find(
                  ({ _id }) => _id !== aquilaConfigObj._id
                )
