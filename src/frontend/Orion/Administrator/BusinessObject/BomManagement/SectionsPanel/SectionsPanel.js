...
const getSectionField = (section) => {
  const sectionIdObj = section ? { sectionId: section._id } : {}

  const fieldObj =
    section && section.fields && section.fields[0]
...
const SectionsPanel = () => {
  ...
  const selectedBom = data.bomConfigs.find(({ _id }) => _id === selectedBomId)

  const tabs = selectedBom ? selectedBom.tags : []

  const selectedTab = tabs.find(({ _id }) => _id === selectedTabId)
        ...
        {sections.map((sectionObj) => (
          ...
          >
            ...
            <DeleteButton
              ...
              afterMutationHook={() => {
                const nextSectionSelection = sections.find(
                  ({ _id }) => _id !== selectedSectionId
                )
