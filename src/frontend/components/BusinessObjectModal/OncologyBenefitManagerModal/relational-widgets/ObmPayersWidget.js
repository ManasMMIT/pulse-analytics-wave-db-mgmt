...
const ObmPayersWidget = ({ entity }) => {
  ...
  const payerDropdownOptions = payersData.payerOrganizations
    .map(({ _id, organization }) => ({ value: _id, label: organization }))
            ...
            <div key={_id} style={{ display: 'flex', border: '1px solid black', padding: 12, alignItems: 'center' }}>
              <div style={{ display: 'flex', width: 400, alignItems: 'center' }}>
                ...
                <Select
                  ...
                  onChange={({ value }) => {
                    const newDoc = _.merge(clonedStagedConnections[idx], { payerId: value })
