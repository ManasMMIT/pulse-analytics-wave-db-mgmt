import getChildNodes from '../team/utils/cascadeUpdateResources/getChildNodes'
...
const updateTdgTimestamps = async (
  ...
  await session.withTransaction(async () => {
    ...
    const childNodeIds = getChildNodes(_id, nodesByParentId).map(
      ({ _id: nodeId }) => nodeId
    )
    ...
    const updateCoreNodesOp = pulseCoreDb.collection('nodes').updateMany(
    ...
    const updateCoreRolesOp = pulseCoreDb.collection('roles').updateMany(
      {},
      {
        $set: {
    ...
    const updateDevUsersSitemapsOps = [
      'tools',
      'dashboards',
      'pages',
      'cards',
    ].map((nodeType) =>
      pulseDevDb.collection('users.sitemaps').updateMany(
        {
