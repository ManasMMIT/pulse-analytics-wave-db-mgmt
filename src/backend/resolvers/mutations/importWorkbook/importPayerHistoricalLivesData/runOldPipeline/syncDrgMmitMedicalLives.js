...
const latestMonthYearAggPipeline = require('./latest-month-year-agg-pipeline')
...
const synchronizeLives = async (pulseCoreDb) => {
  ...
  const latestMmitData = await mmitCollection
    .aggregate(latestMonthYearAggPipeline)
    .toArray()

  const mmitSlugToOrgMap = _.mapValues(
    _.keyBy(latestMmitData, 'slug'),
    'organization'
  )
  ...
  const latestMonthsAndSlugMatchQuery = [
    ...
    ...latestMonthYearAggPipeline,
  ]

  const latestDrgData = await pulseCoreDb
    .collection(DRG_COLLECTION)
    .aggregate(latestMonthsAndSlugMatchQuery)

  const iterateDrgCursor = () =>
    new Promise((resolve, reject) => {
      let updatedDocsCount = 0
      let insertionCount = 0

      latestDrgData.forEach(
        (drgRow) => {
          const {
            parentSlug,
            slug,
        ...
        (err) => {
          ...
          console.log(
            `Num of untouched MMIT docs (no corresponding DRG slug): ${
              startingMmitRowsCount - updatedDocsCount
            }`
          )
