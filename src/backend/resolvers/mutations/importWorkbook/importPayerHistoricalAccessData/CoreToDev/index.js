...
const { formatQualityAccessDoc, formatCombinedDataDoc } = require('./formatters')
...
class CoreToDev {
  ...
  async getNonLivesCollectionDocs() {
    ...
    const qualityAccessSixMonthOp = this.pulseCore
      ...
      .aggregate(qualityAccessAggPip(6), { allowDiskUse: true })
      .toArray()

    const additCritSixMonthOp = this.pulseCore
      .collection('organizations.treatmentPlans.history')
      .aggregate(additionalCriteriaAggPip(6), { allowDiskUse: true })
      .toArray()

    const qualityAccessOneMonthOp = this.pulseCore
      .collection('organizations.treatmentPlans.history')
      .aggregate(qualityAccessAggPip(1), { allowDiskUse: true })
      .toArray()

    const addCritOneMonthOp = this.pulseCore
      .collection('organizations.treatmentPlans.history')
      .aggregate(additionalCriteriaAggPip(1), { allowDiskUse: true })
      .toArray()

    const policyLinkSixMonthOp = this.pulseCore
      .collection('organizations.treatmentPlans.history')
      .aggregate(policyLinkAggPipeline(6), { allowDiskUse: true })
      .toArray()

    const policyLinkOneMonthOp = this.pulseCore
      .collection('organizations.treatmentPlans.history')
      .aggregate(policyLinkAggPipeline(1), { allowDiskUse: true })
      .toArray()
    ...
    const formattedQualityAccessLastSixMonthsDocs = qualityAccessLastSixMonthDocs.map(
    ...
    ] = qualityAccessLastMonthDocs.reduce(
      (acc, doc) => {
  ...
  async materializeQualityAccess() {
    const {
      qualityAccess: { htDocs, nonHtDocs },
    } = this.nonLivesCollectionDocs
    await this.pulseDev.collection('payerHistoricalQualityAccess').deleteMany()

    await this.pulseDev.collection('payerHistoricalQualityAccessHt').deleteMany()

    if (nonHtDocs.length) {
      await this.pulseDev.collection('payerHistoricalQualityAccess').insertMany(nonHtDocs)
    }

    if (htDocs.length) {
      await this.pulseDev.collection('payerHistoricalQualityAccessHt').insertMany(htDocs)
    }
  }

  async materializeAdditionalCriteria() {
    const {
      additionalCriteria: { htDocs, nonHtDocs },
    } = this.nonLivesCollectionDocs

    await this.pulseDev.collection('payerHistoricalAdditionalCriteria').deleteMany()

    await this.pulseDev.collection('payerHistoricalAdditionalCriteriaHt').deleteMany()

    if (nonHtDocs.length) {
      await this.pulseDev.collection('payerHistoricalAdditionalCriteria').insertMany(nonHtDocs)
    }

    if (htDocs.length) {
      await this.pulseDev.collection('payerHistoricalAdditionalCriteriaHt').insertMany(htDocs)
    }
  }

  async materializePolicyLinks() {
    const {
      policyLink: { htDocs, nonHtDocs },
    } = this.nonLivesCollectionDocs

    await this.pulseDev.collection('payerHistoricalPolicyLinks').deleteMany()

    await this.pulseDev.collection('payerHistoricalPolicyLinksHt').deleteMany()

    if (nonHtDocs.length) {
      await this.pulseDev.collection('payerHistoricalPolicyLinks').insertMany(nonHtDocs)
    }

    if (htDocs.length) {
      await this.pulseDev.collection('payerHistoricalPolicyLinksHt').insertMany(htDocs)
    }
  }
  ...
  async materializeCombinedNonLivesData() {
    ...
    await this.pulseDev.collection('payerHistoricalCombinedData').deleteMany()

    if (combinedData.length) {
      await this.pulseDev.collection('payerHistoricalCombinedData').insertMany(combinedData)
    }
  }
  ...
  async materializeNonLivesCollections() {
    ...
    console.log(
      'quality access, additional criteria, policy links, and combined collection docs finished formatting'
    )
