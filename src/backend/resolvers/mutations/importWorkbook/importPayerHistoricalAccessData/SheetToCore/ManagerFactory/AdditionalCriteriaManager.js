...
class AdditionalCriteriaManager extends Manager {
  ...
  getCriteriaData(additionalCriteriaDocs) {
    return additionalCriteriaDocs.map(
      ({ criteriaLabel, criteria, criteriaNotes, restrictionLevel }) => {
        return {
  ...
  getPermittedOps() {
    ...
    return Object.keys(dataGroupedByOrgTp).map((orgTpId) => {
      orgTpId = ObjectId(orgTpId)

      const additionalCriteriaDocs = dataGroupedByOrgTp[orgTpId]

      const { treatmentPlanId, organizationId } = additionalCriteriaDocs[0]

      const additionalCriteriaData = this.getCriteriaData(
        additionalCriteriaDocs
      )
