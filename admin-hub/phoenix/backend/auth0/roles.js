...
class RoleDao {
  ...
  async update({ id, name }) {
    try {
      ...
      const newName = `${clientName}-${name}`

      const nameAndDescrip = { name: newName, description: name }
  ...
  async delete({ id, clientId }) {
    try {
      ...
      await Promise.all([
        this.authClient.removeNestedGroup(clientId, id),
        this.authClient.removeGroup(id),
        this.authClient.removeRole(roleIdToDelete),
      ])
